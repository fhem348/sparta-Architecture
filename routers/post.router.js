...
router.patch('/:postId', jwtValidate, async (req, res, next) => { //수정
  ...
  if(!title||!content||!image){
    return res.status(400).json({ success: false, message: '수정사항 양식에 맞지 않습니다.'})
  }
  ...
  if (!userPost){
    return res.status(400).json({ success: false, message: '존재하지 않는 Post입니다.'})
  }
  ...
  res.status(401).json({ success: false, message: "수정이 완료되지 않았습니다." });
}});
...
router.delete('/:postId', jwtValidate, async (req, res, next) => { //삭제
  ...
  if(!post){
    return res.json({ success: false, message: 'postId가 존재하지 않습니다.' })
  }
...
router.post('/:postId/like', jwtValidate,async (req, res, next) => { //좋아요
  ...
  if (post.userId === userId){
    return res.json({ success: false, message: '작성자는 좋아요를 누를 수 없습니다.'})
  }
  ...
  return res.status(201).json({ success: true, message: '좋아요가 적용되었습니다.'});
}catch(err){
  res.status(401).json({ success: false, message: "좋아요가 적용되지 않았습니다." });
}});
